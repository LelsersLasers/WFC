        ...
        for x in start_x..end_x {
            for y in start_y..end_y {
                let (x, y, m) = if args.wrap_input {
                    ...
                    if x >= src.width() as i32 {
                        m |= 1 << 1;
                    }
                    if y < 0 {
                        m |= 1 << 2;
                    }
                    if y >= src.height() as i32 {
                        m |= 1 << 3;
                    }
                    (x, y, m)
                };
    ...
    fn can_go_next_to(&self, other: &Self, offset_x: i32, offset_y: i32, n: usize) -> bool {
        for pattern_x in 0..n as i32 {
            for pattern_y in 0..n as i32 {
                ...
                if other_pattern_x < 0
                    || other_pattern_x >= n as i32
                    || other_pattern_y < 0
                    || other_pattern_y >= n as i32
                {
                    continue;
                }
                let color = self.color_at(pattern_x as usize, pattern_y as usize, n);
                let other_color =
                    other.color_at(other_pattern_x as usize, other_pattern_y as usize, n);
...
impl PartialEq for Overlap {
    fn eq(&self, other: &Self) -> bool {
        self.pattern_idx == other.pattern_idx
...
impl GridSpot {
    fn new(x: i32, y: i32, patterns_length: usize) -> Self {
        Self {
            x,
            y,
            valid_patterns: vec![true; patterns_length],
        }
    }
    fn collapse(&mut self) {
        let valid_idxs: Vec<usize> = self
            .valid_patterns
            .iter()
            .enumerate()
            .filter_map(|(i, state)| if *state { Some(i) } else { None })
            .collect();
        ...
        for (i, pattern) in patterns.iter().enumerate() {
            ...
            for x in 0..args.n as i32 {
                for y in 0..args.n as i32 {
                    ...
                    if output_x >= args.dims_x as i32 {
                        m |= 1 << 1;
                    }
                    if output_y < 0 {
                        m |= 1 << 2;
                    }
                    if output_y >= args.dims_y as i32 {
                        m |= 1 << 3;
                    }
...
impl Wave {
    ...
    pub fn step(&mut self) {
        ...
        let knotted = self
            .grid
            .par_iter()
            .any(|spot| spot.valid_patterns.iter().filter(|&&x| x).count() == 0);
    ...
    fn propagate(&mut self) {
        ...
        let mut sorted_offsets = offsets
            .clone()
            .flat_map(|x| offsets.clone().map(move |y| (x, y)))
            .filter(|(x, y)| *x != 0 || *y != 0)
            .collect::<Vec<(i32, i32)>>();
        sorted_offsets.sort_unstable_by_key(|(x, y)| x.pow(2) + y.pow(2));
        ...
        sorted_offsets
            ...
            .for_each(|(offset_x, offset_y)| {
                ...
                if other_pos.0 < 0
                    || other_pos.0 >= self.args.dims_x as i32
                    || other_pos.1 < 0
                    || other_pos.1 >= self.args.dims_y as i32
                {
                    return;
                }
                ...
                let other_possible_patterns = (0..pattern_len)
                    .flat_map(|i| {
                ...
                let new_valid_patterns = old_valid_patterns
                    .iter()
                    .enumerate()
                    .map(|(i, state)| {
                        if !*state {
